...
const AUTOSAVE_EVERY = 100;   // words between auto-saves during playback
...
const Store = {
  ...
  get(title) {
    return this.all().find(s => s.title === title) ?? null;
  },
  remove(title) {
    this._write(this.all().filter(s => s.title !== title));
  }
};
...
const UI = {
  ...
  syncPlayBtn() {
    this.playPause.textContent = Player.playing ? '⏸' : '▶';
  },
  ...
  renderSessionList() {
    ...
    sessions.forEach(s => {
      const pct = s.totalWords > 1
        ? Math.round(s.wordIndex / (s.totalWords - 1) * 100)
        : 0;
      ...
      item.innerHTML = `
        ...
        <div class="session-bar-track">
          <div class="session-bar-fill${pct >= 100 ? ' complete' : ''}"
               style="width:${pct}%"></div>
        </div>
      ...
      item.querySelector('.session-del').addEventListener('click', e => {
        e.stopPropagation();
        Store.remove(s.title);
        this.renderSessionList();
      });

      item.addEventListener('click', () => triggerFilePick());
    ...
    addBtn.addEventListener('click', triggerFilePick);
...
function wireEvents() {
  ...
  UI.loadBtn.addEventListener('click', triggerFilePick);
  UI.dropZone.addEventListener('click', triggerFilePick);
  ...
  document.addEventListener('dragleave', e => {
    if (!e.relatedTarget) UI.dropZone.classList.remove('dragover');
  });
  ...
  UI.backFromError.addEventListener('click', () => {
    Player.stop();
    UI.showLibrary();
  });
  ...
  document.addEventListener('keydown', e => {
    if (UI.rdrView.classList.contains('hidden')) return;
    if (e.target.tagName === 'BUTTON') return;   // don't intercept focused buttons
    switch (e.key) {
